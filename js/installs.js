...
MP.api.segment('App_install', paramsToday).done(function(installsResults) {
    ...
    var todaysInstalls = addCommas(installs.App_install[paramsToday.to.format("YYYY-MM-DD")])
    // replace text in header pannel to show today's installs
    $('#installs-today').text(todaysInstalls)

    //graph monthly data using MPChart
    var newInstalls = $('#new-installs').MPChart({chartType: 'line'});   //create the chart and attach to the new installs dive                             
